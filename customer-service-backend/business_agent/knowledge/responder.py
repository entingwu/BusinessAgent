...
class KnowledgeResponder:
    ...
    async def respond(self,
        ...
        # 3. 内部记录命中的分片 ID 与相似度，回复可溯源
        logger.info(
            "knowledge_respond hits=%s traces=%s",
            len(selected), [chunk.trace() for chunk in selected],
        )
    ...
    async def respond_unavailable(self, state: DialogueState) -> list[BotMessage]:
        """
        Goal: 检索链路不可用时的降级回复。不调用 LLM，不使用模型自身知识作答。
        Args:
            state: 对话状态（保留参数，便于后续接人工接管状态）
        Returns: list[BotMessage]
        """
        logger.warning("knowledge_respond fallback=unavailable sender_id=%s", state.sender_id)
        return [BotMessage(text=FALLBACK_UNAVAILABLE_TEXT)]

    def _select_chunks(self, chunks: list[KnowledgeChunk]) -> list[KnowledgeChunk]:
        """
